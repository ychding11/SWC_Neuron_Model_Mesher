...
class MakeNeuronMeta_From_File_Operator ( bpy.types.Operator ):
...
class MakeNeuronMetaPropGroup(bpy.types.PropertyGroup):
    ...
    def delete_vertex_spheres(self, context):
        ...
        if ob_active == None:
            ...
            context.active_objects = vals[0]
            vals[0].hide_set(False)
            vals[0].select_set(True)
        else:
            # Ensure it is also visible and selected
            ob_active.hide_set(False)
...
CableModelRemove,
CableModelRemoveAll,
MakeNeuronMeta_From_File_Operator,
...
if __name__ == "__main__":
  print ("Command line mode, Registering...")
  register()
